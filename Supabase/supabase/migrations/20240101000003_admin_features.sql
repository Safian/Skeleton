...
DROP POLICY IF EXISTS "Admins manage ai_models" ON public.ai_models;
CREATE POLICY "Admins manage ai_models" ON public.ai_models
  FOR ALL TO authenticated
  USING (public.is_admin(auth.uid()))
  WITH CHECK (public.is_admin(auth.uid()));

DROP POLICY IF EXISTS "Service role full access ai_models" ON public.ai_models;
...
DROP POLICY IF EXISTS "Admins read gpt_usage_logs" ON public.gpt_usage_logs;
CREATE POLICY "Admins read gpt_usage_logs" ON public.gpt_usage_logs
  FOR SELECT TO authenticated
  USING (public.is_admin(auth.uid()));

DROP POLICY IF EXISTS "Service role full access gpt_logs" ON public.gpt_usage_logs;
...
DROP POLICY IF EXISTS "Admins read app_error_logs" ON public.app_error_logs;
CREATE POLICY "Admins read app_error_logs" ON public.app_error_logs
  FOR SELECT TO authenticated
  USING (public.is_admin(auth.uid()));

DROP POLICY IF EXISTS "Authenticated users insert own logs" ON public.app_error_logs;
CREATE POLICY "Authenticated users insert own logs" ON public.app_error_logs
  FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid() OR user_id IS NULL);

DROP POLICY IF EXISTS "Service role full access error_logs" ON public.app_error_logs;
...
DROP POLICY IF EXISTS "Admins read all push tokens" ON public.user_push_tokens;
CREATE POLICY "Admins read all push tokens" ON public.user_push_tokens
  FOR SELECT TO authenticated
  USING (public.is_admin(auth.uid()));

DROP POLICY IF EXISTS "Service role full access push_tokens" ON public.user_push_tokens;
...
DROP POLICY IF EXISTS "Admins manage push_notification_logs" ON public.push_notification_logs;
CREATE POLICY "Admins manage push_notification_logs" ON public.push_notification_logs
  FOR ALL TO authenticated
  USING (public.is_admin(auth.uid()))
  WITH CHECK (public.is_admin(auth.uid()));

DROP POLICY IF EXISTS "Service role full access push_logs" ON public.push_notification_logs;
